...
<?php
	if(isset($_POST['MyHead'])) {
		...
		//$sql = "SELECT * FROM course_table WHERE course_id NOT IN(SELECT course_id FROM takes_table WHERE stud_id LIKE \"".$MyHead."%\");";
        $sql = "SELECT DISTINCT takes_table.course_id ,title ,dept_name ,grade ,category ,now_people ,max_people FROM takes_table INNER JOIN course_table ON takes_table.course_id = course_table.course_id  WHERE takes_table.course_id NOT IN (SELECT course_id FROM takes_table WHERE stud_id LIKE \"".$MyHead."%\") ORDER BY takes_table.course_id;";
		...
		while($row = mysqli_fetch_array($result)){
			if($row['now_people']==$row['max_people'])  //滿額
			{
