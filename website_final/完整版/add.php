<?php
	if(isset($_GET['m'])) {
		...
		echo "<a href = 'action1.php?m=$MyHead'>回可選課表</a> <p>";
		...
		$sql_already = "SELECT takes_table.course_id ,title ,dept_name ,takes_table.grade ,category ,now_people ,max_people FROM takes_table INNER JOIN course_table ON takes_table.course_id = course_table.course_id where stud_id LIKE \"".$MyHead."%\"ORDER BY takes_table.course_id;";
		...
		// course credits
		$sql_coursecred = "SELECT credits FROM course_table WHERE course_id = ".$add.";";
		...
		if($def==0)
		{
			...
			//關注清單
			$sql_3 = "UPDATE focus_table SET now_people = now_people + 1 WHERE course_id = ".$add.";";
			mysqli_query($conn, $sql_3);
			...
			*/
			$sql_4 = "INSERT INTO takes_table VALUES('{$MyHead}','{$add}','{$add}',2,2023,2);";
